...
- Current locally and independently verified, committed non-dispatch code
  ...
  final local evidence is recorded below. Combined sealed-artifact code/docs
  checkpoint `8e99c4c` is green in complete remote run `33332561043`. Combined
  A-minus code/docs checkpoint `91d77f5` is green in complete remote CI run
  `33330786187`, and remote receipt checkpoint `0abca2f` is green in run
  `33331161012`.
...
surfaces/zero violations, content hashes, and `git diff --check` pass. The
code-only commit was not built as a separate remote head; combined A-minus
code/docs checkpoint `91d77f5`, containing `9ca6822`, is green in complete
remote run `33330786187`. The preceding docs-only checkpoint `b0e240c` is green
...
`git diff --check` are green. Combined code/docs checkpoint `8e99c4c`, which
contains code checkpoint `50f5de7`, is green in complete remote run
`33332561043`. Experiment policy revision 4 retains all seven blockers and
