...
## Policy result registry authority (2026-08-30)

Policy benchmark evidence is now classified by the typed, fail-closed registry
at `evidence/policy/result_registry_v2.json`. Every row binds its effective and
declared pixel caps, CoreDev task SHA, exact weights identity, runtime, parser,
action boundary, observation, prompt, RNG, generation/scorer contracts and
immutable score artifact. The only allowed statuses are `golden`, `standalone`,
`confounded`, `invalid` and `pending`. No currently registered cross-method row
is `golden`.

`tools/materialize_policy_result_table.py` is the sole table materializer. It
verifies registered artifacts by default and emits a numeric delta only when
both rows are `golden`, are fixed members of one independently preregistered
comparison definition, all hard invariants match, and their actual differences
equal that definition's intervention axes exactly; otherwise it emits
`contract differs`. Task/RNG/scorer/coverage identities are hard invariants.
Method, weights, training identity, pixels, runtime, parser, action boundary,
observation, prompt and generation remain fully recorded but may be registered
as treatment axes instead of being unconditional invariants. Every undeclared
extra difference fails closed. The current v2 registry has no comparison
definitions, so changing result-row labels alone cannot authorize a delta.
Pixel equality by itself is insufficient. In particular, PRL26 No-Tool and
Crop share 2,240 support IDs but have **0/2,240 intersecting RNG identities**,
as well as different prompts, runtimes and continuations. Their aggregate
receipt is an operational two-arm handoff record, not a canonical or paired
A/B comparison.
...
from that lineage are withdrawn. `c448e583`/PRL27-B is the first canonical
single-renderer live/replay implementation, but it stopped at S4; S32 and formal
...
### Crop continuation incident, invalid PRL27-A and canonical PRL27-B (2026-08-30)
...
- Invalid attempted correction: `ecddc379` changed the live Crop appender to
  the 60-token continuation (text SHA256
  `f745fa6cfcc3ba9eb27125a49581fd823fb5930b7b0a51b28e51982999fa2d0a`)
  but left immutable replay layout on generic86. PRL27-A was therefore a
  live60/replay86 hybrid. Its checkpoint, reward and status artifacts are all
  `invalid`; the `122db51` binder/config does not make that lineage corrected,
  and no PRL27-A score may enter a paper table.
- Canonical implementation: `c448e583` is the first version that selects one
  success renderer and passes the exact same rendered bytes to both live
  appender and replay layout. This PRL27-B lineage retains `training_run`,
  precomputed image embeddings, strict parser, canonical JSON errors, six-call
  cap, total-response accounting and a 10,240-token per-turn ceiling. It is the
  only admissible starting point for a future matched60 Crop rerun.
- Execution boundary: PRL27-B stopped at S4. It has no S32 checkpoint and no
  formal CoreDev evaluation, so the requested endpoint remains `pending`.
  Stabilization work does not authorize resuming it. A future launch must use a
  fresh, explicit run/evaluation identity and cannot inherit PRL27-A artifacts.
