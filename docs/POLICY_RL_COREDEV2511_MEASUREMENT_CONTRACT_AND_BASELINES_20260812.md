...
2. canonical 大表的汇报结构；实验结果持续按同一契约追加，当前更新至 2026-08-15。
...
Crop Step 0/8/16 与 PRL21 Crop T-free Step 8/16 的同标准 `legacy-RNG` 结果位于第 3.1 节；RP67 T-free
...
六条主要轨迹的一页摘要如下。Delta 只在同一行、同一 RNG/protocol block 内计算；
