...
>
> **S1 硬闭环（2026-08-30 02:18:02 JST）**：PRL27-B 已完成 256 条 trajectory 的完整
> rollout、多次 Crop、final replay、反向更新、8-way model/optimizer checkpoint 与 metric 发布，
> 未出现 replay/parse/execution 错误。S1 answer reward 为 `0.67578125`，工具调用题率
> `0.8125`，446 次调用产生 439 个成功 observation；仅记录 7 次调用上限错误。S1 用时
...
>
> **S3 与历史同 step 对照（02:38:14 JST）**：S3 继续完整闭环，tracker=3。相比历史
> continuation-mismatched PRL26-B 的前三步累计，新训练 answer reward 从 `0.7421875` 降到
> `0.7330729`，format error 从 `0.0924479` 升到 `0.1341146`；这是早期质量侧退化，不能被
> 更快吞吐掩盖。与此同时工具调用题率 `0.8320313→0.8333333`、成功 observations
> `1546→1551`，说明工具探索没有塌缩。S3 单步调用 633 次、成功 observation 602 个、answer
> reward `0.8203125`、format error `0.1875`，没有 replay/execution failure。当前只能记为
> “工具覆盖持平，answer 略低，格式/协议明显较差，速度更快”，等待 S4/S8 与最终评测确认。
>
> **S4 趋势确认（02:49 JST）**：四步累计相对历史 PRL26-B，answer reward
> `0.7539063→0.7294922`、format error `0.0976563→0.1425781`，质量差距继续扩大；工具调用题率
> `0.8242188→0.8271484`、成功 observations `2051→2105`，工具覆盖不降反升。S4 单步调用
...
>
> **插件修复的真实 GPU 闭环（03:42--03:43 JST）**：四个 benchmark rank 于
> 03:42:47 同时存活，四个 EngineCore 于 03:42:59--03:43:00 启动并全部解析为
> `TGVFQwen3VLForConditionalGeneration`。03:43:18--03:43:21 四个 worker 均成功注入
> `TGVFVLLMWorkerExtension`，03:43:22--03:43:25 四个 worker 均确认
> `AttentionBackendEnum.TRITON_ATTN`。03:43:33--03:43:42，GPU 0--3 各自完成同一 S32
> full-model 加载，每个模型占用 `17.3668 GiB`；03:43:48 资源快照为 GPU 0--3
> 各 `24,960 MiB`，GPU 4--7 仍空闲。这已越过初次 attempt 的 unsupported-architecture
> 失败点，但当前仍只能记为 inference running：尚未核验任何正式 row，也没有
> benchmark 结果或分数。
>
> **Denominator correction 与最新推理快照（03:57 JST）**：前一版把运行进度
> 误写为 `659/2511` 和 `853/2511`。Frozen manifest 的确共有 2,511 条，但已冻结
> plan 明确分为 2,240 条 supported single-image 与
> `unsupported_multi_image_count=271`；这 271 条 multi-image 的 protocol decision 仍为
> pending，不进入本次 Crop policy inference。因此正确快照是 03:46:35 的
> `659/2240` 与 03:47:45 的 `853/2240`，不是以 2,511 为 worker denominator。
>
> 03:46 左右的 761-row 只读快照中，667 题使用工具（`87.65%`），共 694 次
> 调用、3 个 typed tool error；stop 类别为 `direct=93`、`final=667`、
> `call_cap=1`。03:48 资源快照时 GPU 0--3 每卡约占用 `145 GB` 且均在推理；
> 至 03:57，进度为 `2125/2240 = 94.9%`，rank 1/3 已正常完成并释放对应
...
> GPU 0--3 全部释放。这 2,240 条 supported single-image 中，尝试工具的 trajectory
> 为 `1561` 条（`69.6875%`），至少一次成功的为 `1557` 条（`69.5089%`），
> official `no_tool=679`；成功 calls/observations `1616`，typed tool errors `6`（包含失败尝试时
> total attempts 为 `1622`）。
> stop 分布为 `direct=673`、`final=1551`、`max_tokens=15`、`call_cap=1`，单题调用次数
> 分布为 `0:683, 1:1521, 2:21, 3:10, 4:3, 5:1, 6:1`；这里是 **successful-call-count**
> 分布，先前 jq 粗算的 `683` 是“0 个 successful call”，不是 official no-tool 定义。
> 271 条 multi-image 继续作为
...
> `2240/2511 + 271 held multi-image`。权威结果为
...
plugin 修复和 CPU canary 通过后，recovery2 已于 03:38:39 从独立 control 启动。
...
| **Crop** | Native RGB Crop；现有主表 S80 值是 60-token cross-protocol 历史测量 | PRL26-B S32 owner-native 86-token Eval@512 已 complete/pass；PRL25-B S80 保留历史曲线 | PRL26-B S32 aligned Macro* `53.5548`；旧 60-token S32 为 `54.9351` cross-protocol；PRL27-B 仅为已停 S4 negative ablation；不补 seed43 |
...
† Crop S80 使用已确认 continuation-mismatched 的历史训练合同；表中保留其测量值用于追溯，不得把它
称为 train/eval-aligned golden 或用于归因 Crop 工具本身。训练原生 86-token 的 PRL26-B
S32@512 aligned 结果现已 complete/pass（Macro* `53.5548`），与这一 S80 历史行必须分开。
