...
更新时间：2026-08-30（Asia/Tokyo；repository stabilization audit）

> **阻断性更正——Crop post-tool continuation 与 replay geometry（2026-08-30）：**
> PRL25-B 与 PRL26-B 的 Crop-only 训练在成功 Crop 后追加 generic86 continuation，而此前
> native evaluator 追加的是 60-token Crop continuation；这些数值只能保留为历史证据。
> 更重要的是，`ecddc379`/PRL27-A 只把 live appender 改成 matched60，replay layout 仍按
> generic86 定位，因此它是 **live60/replay86 hybrid**，不是 corrected run。PRL27-A 产生的
> checkpoint、reward 与进度状态全部标为 `invalid`，且没有可接受的评测结果。
>
...
## Registry-generated true-1M evidence inventory（mixed contracts）

五行共享 true-1M 像素上限与 CoreDev task SHA，但 v2 registry 当前没有任何独立
`comparison_definitions`；结果行自身携带的历史分组标签或 intervention axes 不能构成预注册。
实际存在的 weights、训练合同、prompt、runtime、parser、action boundary、observation 与 RNG
差异均未被预声明为单一实验干预，因此没有任何跨方法 numeric delta，也没有 `golden` 行。下面的表由
`evidence/policy/result_registry_v2.json` 经默认 artifact 校验后生成；编辑 registry，
不要手改数值或补算差值。

<!-- Generated by tools/materialize_policy_result_table.py; edit the registry, not this table. -->

### true-1M evidence inventory (mixed contracts; no cross-method delta)

| Result | Status | Train px | Declared eval px | Effective eval px | Macro* | Score evidence | Δ vs reference | Contract |
|---|---|---:|---:|---:|---:|---|---:|---|
| Original raw-direct true-1M | standalone | — | 1,003,520 | 1,003,520 | 61.3147 | content-checked | reference | `191c49b65459` |
| No-Tool RL S32 true-1M | standalone | 1,003,520 | 1,003,520 | 1,003,520 | 63.7520 | content-checked | — (contract differs) | `e809840abf33` |
| Crop S32 true-1M (historical) | confounded | 1,003,520 | 1,003,520 | 1,003,520 | 61.0706 | content-checked | — (contract differs) | `e1b4c6732bf1` |
| TGVF S64 true-1M | standalone | 1,003,520 | 1,003,520 | 1,003,520 | 59.8086 | digest-only | — (contract differs) | `76aeb5b4971b` |
| Atomic S16 true-1M | standalone | 1,003,520 | 1,003,520 | 1,003,520 | 63.0827 | digest-only | — (contract differs) | `b2c4e02028cb` |
...
### Registry-generated @512 evidence inventory（mixed contracts；not A/B）

<!-- Generated by tools/materialize_policy_result_table.py; edit the registry, not this table. -->

### @512 and nominal-@512 evidence inventory (mixed contracts; not A/B)

| Result | Status | Train px | Declared eval px | Effective eval px | Macro* | Score evidence | Δ vs reference | Contract |
|---|---|---:|---:|---:|---:|---|---:|---|
| Original raw-direct @512 | standalone | — | 262,144 | 262,144 | 55.3556 | digest-only | reference | `01632dd58be7` |
| No-Tool PRL26-A S32 @512 | standalone | 262,144 | 262,144 | 262,144 | 59.0506 | digest-only | — (contract differs) | `edff6211559c` |
| Crop PRL26-B S32 @512 (official60) | confounded | 262,144 | 262,144 | 262,144 | 54.9351 | content-checked | — (contract differs) | `bc73c266f63f` |
| Crop PRL26-B S32 @512 (exact generic86 runtime) | standalone | 262,144 | 262,144 | 262,144 | 53.5548 | content-checked | — (contract differs) | `16434becc672` |
| TGVF S64 @512 | standalone | 1,003,520 | 262,144 | 262,144 | 55.4067 | digest-only | — (contract differs) | `10847a2f26dd` |
| Atomic S16 @512 | standalone | 1,003,520 | 262,144 | 262,144 | 57.2762 | digest-only | — (contract differs) | `c9e25b33a919` |
| Crop S80 pre-boundary (processor-default) | invalid | 1,003,520 | 262,144 | 16,777,216 | 61.5591 | content-checked | — (contract differs) | `302fb089209d` |
| Crop S80 boundary-fix (processor-default) | confounded | 1,003,520 | 262,144 | 16,777,216 | 62.0967 | content-checked | — (contract differs) | `152f137e0bac` |
...
真正的 @512 行具有 `262,144` effective cap；两个 S80 nominal-@512 行的轨迹审计则证明 effective
cap 实为 `16,777,216`。PRL26 No-Tool 与 Crop 虽共享 2,240 个支持 ID，但 2,240/2,240 条 RNG
identity 的交集为 **0**，并且 prompt、agent loop 与 continuation 不同，因此不是 paired/canonical
A/B。它们的数值只能按各自合同独立报告，不能计算或解释 No-Tool−Crop delta。

PRL26-B official60 的 512 pixel cap 已验证，但历史 artifact 没有密码学绑定 action-boundary
identity，这也不能补偿训练/评测 continuation mismatch；generic86 evaluation 只恢复该历史训练 runtime 的 standalone 可解释性，
也不能与 official60 或 No-Tool 构成 prompt A/B。
...
### Method-specific resolution endpoints（different contracts）

| Method | true-1M Macro* | true-512 Macro* | Registry delta decision | 边界 |
|---|---:|---:|---|---|
| TGVF S64 | 59.8086 | 55.4067 | **refused: eval pixel contract differs** | same run/step/RP67/prompt/tool/task/RNG/scorer；snapshot semantic equality 未闭合 |
| Atomic S16 | 63.0827 | 57.2762 | **refused: eval pixel contract differs** | same run/step/RP67/prompt/tool/task/RNG/scorer；snapshot semantic equality 未闭合 |

Results 层面只保留四个 endpoint 原值，不生成 512−1M delta。Discussion 层面，这一 pattern
与 TGVF-family 对 policy-RL 训练/评测尺度对齐的依赖相容，但不是因果证明。RP67 adapter 预训练为 `512²`，所以此处
...
不能再用于补算 resolution delta 或“Crop 在 constrained-pixel setting 最优”的论断。要闭合该问题，
...
保留为 historical，aligned Crop 等待 PRL27-B canonical lineage 从 S0 完成重训重测。旧 No-Tool
...
PRL27-A 已因 live60/replay86 hybrid 判为 invalid；PRL27-B 是首个 canonical 实现但停在 S4。
TGVF Short S32 只有训练 receipt、尚无正式评测；Target-guide-v2 与 Atomic 的正式 endpoint 也
仍为 pending。训练 reward 和工具调用统计只作健康与机制诊断，不替代七项 benchmark。
...
| TGVF Target-guide-v2 Train@512 | **formal S32/Eval pending** | 只增加 teacher-aligned Target 定义与视觉案例 | 实验冻结 |
| Atomic Train@512 | **formal root absent；pending** | 停止的 waiter 不是训练证据 | 不自动接力或重启 |
...
bbox 语义质量与工具 abstention。当前最低闭环首先是 PRL27-B canonical lineage 的 fresh-S0
S32@512 重训与同合同评测；PRL27-A hybrid 不得复用。之后再决定是否补 PRL25 Crop S32 真正 Eval@512、PRL26 Crop S32
...
true-1M 结果另见文首 registry evidence inventory。与 true@512 的严格因果解释仍受 RNG/protocol 是否配对的边界
约束。
...
| **Crop** | PRL25-B native RGB Crop（continuation-mismatched historical） | S32/S80，seed42 | true-1M S32 `61.0706` 与 S80 `59.6463` 只作历史测量；aligned endpoint 等待 PRL27-B canonical lineage；不补 seed43 |
...
## 3. 历史 mixed-contract 参考表（已被文首 registry evidence inventory supersede）
...
No-Tool/Crop 的历史 two-arm evaluator 使用相同 2,240 个支持 ID、七项 CoreDev-2511 scorer 与
@512 processor；两臂都是 temperature 1、master seed42，但 2,240/2,240 条 RNG identity 的交集
为 **0**。后续 continuation 审计还证明 Crop arm 并非 train/eval matched，因此该 operational
handoff 既不能估计逐样本 common-random-number paired effect，也不能作为工具因果比较。两臂
运行完成后，第二个 relay 先
...
同时把两臂 result、operational handoff identity、精确 `2,511/2,240/271/7` 覆盖、两条正式 summary
...
`training-resources-released.json`。历史两臂 operational handoff identity（不是科学比较合同）为
...
但 post-tool continuation 并不 matched，不能与未来 canonical Crop 或历史 Crop S80 nominal-@512 混写。
...
GPU。未来 PRL27-B canonical lineage 不复用这条旧 Crop handoff，必须以新身份另行明确启动。任何中间
...
| 广义 prompt bundle 下工具总体增益仍存在 | TGVF / Atomic stress-test Macro* 分别为 `58.5138 / 60.4684` | prompt/observation 合同与 matched、Original 均不同，registry 拒绝跨合同 delta；非 target-only | **总体 claim 不支持；保留分项稳健性** |
| 只补充 target 定义与案例的稳健性 | TGVF / Atomic target-only Macro* 分别为 `58.1788 / 60.8253` | target-only 与 matched prompt 是不同合同；在注册成同一预声明 comparison projection 前不补算 delta | 已支持 benchmark-specific sensitivity，不支持总体增益 |
| 工具方法整体优于 raw direct Original | true-1M standalone Macro*：Original `61.3147`、TGVF `59.8086`、Atomic `63.0827`；Crop `61.0706` 为 confounded | prompt/tool/runtime/parser/RNG 均不同；registry 明确拒绝跨方法 delta，历史 Crop S80 `62.0967` 也不是真正 true-512 | **整体 claim 不支持；等待 canonical Crop rerun** |
| 工具方法的总体增益超越 RL-only/no-tool control | true-1M standalone No-Tool S32 为 `63.7520`；同一 No-Tool 合同内 S32−S0 为 `−0.0098 pp` | No-Tool 与工具方法合同不同，不能计算方法间 delta；Crop 另有 continuation mismatch | **总体 claim 不支持；转向能力切片与机制证据** |
...
| TGVF-family 依赖 policy-RL train/eval scale alignment | TGVF/Atomic 的 true-1M 与 @512 endpoint 分别为 `59.8086/55.4067` 和 `63.0827/57.2762` | 像素本身定义不同 comparison contract，且 snapshot semantic equality 未闭合；registry 不生成 resolution delta，需同 snapshot factorial | **仅为描述性 pattern，非因果证明** |
...
1. **Unified true-1M comparison.** 先报告 Original、frozen No-Tool S32、TGVF S64 与 Atomic
   S16；Crop S32 仅以 `confounded` historical row 保留，待 PRL27-B canonical S32 result 替换。
...
15. [invalid] PRL27-A 是 live60/replay86 hybrid；其 checkpoint、reward、status 全部撤回；
16. [pending] PRL27-B `c448e583` 是首个 canonical renderer/replay 实现，但停在 S4，尚无 S32/Eval；
17. [pending] TGVF Short 只有 S32 receipt、没有正式评测；Target-guide-v2 与 Atomic endpoint 也
    未闭合，且稳定化期间禁止自动接力；
