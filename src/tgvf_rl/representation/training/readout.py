...
class RepresentationReadoutLossSupervision:
    ...
    ``labels`` stays aligned to the native transcript. The component positions
    make answer-bearing supervision auditable without changing the transcript,
    candidate observations, or historical ``ModelEvidenceSupervision``. The
    source-image query range is half-open and identifies causal queries that
...
class RepresentationReadoutRow:
    ...
    def loss_labels(self) -> tuple[int, ...]:
        """Effective labels, preserving historical behavior by default."""
