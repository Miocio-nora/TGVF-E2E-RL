...
def install_deterministic_mcq_answer_policy(module: Any) -> None:
    ...
    judge parses. CoreDev instead binds that fallback to ``Z``, a deterministic
    invalid sentinel, and preserves an auditable sample-level failure marker.
    """
...
def _judge_parse_failure_limit(sample_count: int) -> int:
    """Allow isolated bad rows while stopping systemic scorer failure."""
