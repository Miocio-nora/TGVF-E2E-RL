...
class _DeterministicChoiceProxy:
    """Replace ValKit's sole MCQ random fallback with the invalid sentinel."""
...
def install_deterministic_mcq_answer_policy(module: Any) -> None:
    """Parse CoreDev A--J answers and score exhausted matching as incorrect.

    The pinned ValKit matcher randomly chooses an option after three failed
    judge parses.  CoreDev forbids that behavior: ``Z`` is the deterministic
    invalid sentinel, and the result log remains auditable at sample level.
    """
...
def _deterministic_judge_parse_failures(
    ...
    *,
    ...
    """Audit MCQ result coverage and return deterministic parse-failure ids."""
    ...
    for path in paths:
        ...
        with path.open(encoding="utf-8", newline="") as handle:
            ...
            for row in rows:
                ...
                sample_id = str(row.get("index") or row.get("sample_id") or "").strip()
                if not sample_id:
                    raise RuntimeError(
...
def _judge_parse_failure_limit(sample_count: int) -> int:
    """Allow isolated bad rows, while still stopping systemic scorer failure."""
...
def summarize_coredev_results(
    *,
    ...
    if not isinstance(expected_model, str) or not expected_model.strip():
        raise ValueError("expected model must be non-empty text")
