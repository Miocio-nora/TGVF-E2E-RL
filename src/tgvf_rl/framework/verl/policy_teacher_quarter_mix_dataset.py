...
POLICY_TEACHER_QUARTER_MIX_AGENT_NAME = VERL_SELECTED_SAMPLE_AGENT_NAME
...
def _assert_bound_qwen3_instruct_processor(
    processor: object, binding: "PolicyTeacherQuarterMixDatasetBinding"
) -> None:
    tokenizer = getattr(processor, "tokenizer", None)
    if tokenizer is None or not callable(getattr(tokenizer, "encode", None)):
        raise TypeError("teacher-quarter processor must expose its tokenizer")
    ...
    if not isinstance(template, str) or not template:
        raise ValueError("teacher-quarter Qwen chat template must be explicit")
    if hashlib.sha256(template.encode("utf-8")).hexdigest() != (
        binding.chat_template_sha256
    ):
        raise ValueError("teacher-quarter Qwen chat template identity differs")
    if not callable(getattr(processor, "apply_chat_template", None)):
        raise TypeError("teacher-quarter processor must apply its native chat template")
...
def _render_native_instruct_prompt(
    *,
    ...
    binding: "PolicyTeacherQuarterMixDatasetBinding",
) -> tuple[str, tuple[int, ...]]:
    """Render the bound Instruct prompt without depending on a legacy dataset."""
    ...
    if not isinstance(rendered, str):
        raise TypeError("teacher-quarter chat template did not return text")
    if "<answer>" in rendered or "</answer>" in rendered:
        raise ValueError("teacher-quarter prompt contains a forbidden answer wrapper")
    expected_prefill = "<|im_start|>assistant\n"
    if not rendered.endswith(expected_prefill):
        raise ValueError("teacher-quarter prompt is not Qwen3-VL Instruct native form")
    ...
    if (
        ...
        raise TypeError("teacher-quarter tokenizer returned invalid prompt IDs")
    _assert_bound_qwen3_instruct_processor(processor, binding)
...
def _smoke_expectation(profile: NativeToolCapabilityProfile, data_source: str) -> str:
...
class PolicyTeacherQuarterMixDataset(TGVFDeepEyesOfficialDataset):
    ...
    def __getitem__(self, index: int) -> dict[str, object]:
        ...
        prompt_identity, prompt_version, builder, agent_name, source_route = (
            _visual_prompt_contract(
        ...
        extra_info = {
            ...
            "source_dataset": getattr(sample, "source_dataset", sample.data_source),
