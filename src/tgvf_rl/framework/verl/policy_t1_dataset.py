"""veRL custom dataset for retained T1 ArxivQA Policy-RL rows."""
...
VERL_POLICY_T1_ARXIVQA_DATASET_SCHEMA = "tgvf-verl-policy-t1-arxivqa-v1"
_CONFIG_FIELDS = {
    "schema_version",
    "root",
...
class VerlPolicyT1DatasetBinding:
    root: Path
    ...
    schema_version: str = VERL_POLICY_T1_ARXIVQA_DATASET_SCHEMA
    ...
    def __post_init__(self) -> None:
        ...
        if self.agent_name != VERL_SELECTED_SAMPLE_AGENT_NAME:
            raise ValueError("Policy T1 dataset agent_name differs")
        if self.schema_version != VERL_POLICY_T1_ARXIVQA_DATASET_SCHEMA:
            raise ValueError("Policy T1 veRL dataset schema differs")
        if (
            ...
            raise ValueError("Policy T1 prompt bundle identity differs")

    @property
    def runtime_binding(self) -> PolicyT1RLRuntimeBinding:
        return PolicyT1RLRuntimeBinding(
    ...
    @classmethod
    def from_config(cls, value: object) -> "VerlPolicyT1DatasetBinding":
        mapping = dict(_plain_mapping(value, "data.tgvf_policy_t1_arxivqa"))
        if set(mapping) != _CONFIG_FIELDS:
            raise ValueError("data.tgvf_policy_t1_arxivqa fields differ")
        mapping["decision_stage"] = PolicyT1DecisionStage(mapping["decision_stage"])
        mapping["tool_profile"] = NativeToolCapabilityProfile(mapping["tool_profile"])
        return cls(**mapping)


class TGVFPolicyT1ArxivQADataset(Dataset):
    """Render verified retained ArxivQA rows with the run-bound crop prompt."""
    ...
    def __init__(
        ...
        if processor is None:
            raise TypeError("native Qwen Policy T1 dataset requires a processor")
        if max_samples != -1:
            raise ValueError("Policy T1 dataset forbids secondary max-sample selection")
        binding = VerlPolicyT1DatasetBinding.from_config(
            _config_value(config, "tgvf_policy_t1_arxivqa")
        )
        ...
        if tuple(str(path) for path in paths) != (str(samples_path),):
            raise ValueError("upstream data_files differ from the bound Policy T1 file")
        runtime = load_policy_t1_rl_runtime(
            binding.root, binding=binding.runtime_binding
        )
        if runtime.samples_sha256 != binding.samples_sha256:
            raise ValueError("Policy T1 samples identity differs")
        if runtime.iteration_identity_sha256 != binding.iteration_identity_sha256:
            raise ValueError("Policy T1 iteration identity differs")
...
__all__ = [
    "TGVFPolicyT1ArxivQADataset",
    "VERL_POLICY_T1_ARXIVQA_DATASET_SCHEMA",
    "VerlPolicyT1DatasetBinding",
]
