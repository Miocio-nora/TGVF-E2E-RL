    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(visual, nn.Module) or not any(
            ...
            raise RuntimeError("current Crop actor requires a trainable Qwen vision tower")
        self.engine = engine
        self.model = model
    ...
    def replay_response_logprobs(
        ...
        *,
        ...
        if not response.policy_indices:
            raise ValueError("current Crop replay requires policy-owned response tokens")
    ...
    for index, (record, block) in enumerate(
        ...
        live_blocks.append(
            InjectedVisualBlock(
                ...
                deepstack=tuple(
                    _batched(value) for value in crop.merged_deepstack
                ),
