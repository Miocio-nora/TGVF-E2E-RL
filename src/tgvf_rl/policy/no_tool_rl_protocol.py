"""Exact visual prompt for the full-Qwen direct-only NoTool control.

The control keeps the historical reasoning/final-answer surface while removing
every tool-facing instruction and schema.  Its transcript is one image-bearing
user message and the runtime must execute it in direct-only mode.
"""
...
def build_no_tool_visual_messages(
    ...
    """Return the exact image-bearing, user-only NoTool transcript."""
