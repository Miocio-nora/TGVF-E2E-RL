...
def _teacher_test_hashes(
    ...
    *,
    ...
    return {
        _encoded_image_identity(row.get("image"), cache=image_cache)[1]
        for row in rows
    }
...
def _stable_sample_id(*, train_sha256: str, source_uid: str) -> str:
    ...
    return "policy-teacher-candidate:" + hashlib.sha256(
...
def _link_image(
    *,
    ...
    if existing is not None:
        return existing
    relative = Path("images") / image_sha256[:2] / (
        image_sha256 + _image_suffix(source_path)
    )
...
def materialize_teacher_t1_candidates(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("teacher T1 output_root must remain inside this repository") from error
    ...
    try:
        for source_uid in sorted(grouped):
            ...
            if not reasons:
                try:
                    rendered_question, ground_truth, selection_metadata = _canonical_task(
                        row
                    )
                except ValueError as error:
