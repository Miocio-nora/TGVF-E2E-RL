...
def _load_scoring_queue(
    ...
    # Split only on the JSONL byte delimiter.  ``str.splitlines()`` also
    # treats Unicode NEL/U+2028/U+2029 inside a valid JSON string as a record
    # boundary and can therefore manufacture truncated JSON rows.
...
def _index_path(root: Path, request: Mapping[str, Any]) -> Path:
    ...
    # Preserve resume compatibility with the earlier small canary runs while
    # avoiding a single directory with millions of entries for T1-04.
...
def publish_t1_semantic_judge_manifest(
    ...
    *,
    ...
    This is the canonical post-processing path after bounded-memory execution
    and legacy-result reuse.  It validates the complete queue, every canonical
    index, and every referenced evidence pathname.  Evidence contents were
    validated by reuse or constructed from a fresh accepted response, so a
    separate full evidence audit must not block foreground finalization.
    """
    ...
    # Submit at most 4*workers paths at a time. ThreadPoolExecutor.map over the
    # complete million-row sequence would preserve order but retain millions
    # of Futures; small ordered batches preserve queue order with bounded RAM.
...
def finalize_t1_scoring(
    ...
    path = Path(config_path).resolve()
    # deterministic-v3 was produced only after the expensive source and raw
    # generation checks.  Finalization consumes that immutable materialization
    # directly; repeating the 77-GB source scan and generation-chunk scan here
    # cannot add information to the final semantic merge.
    run = load_t1_run_config(path, verify_data_files=False)
    _validate_prepared_output_root(run, path)
    ...
    if (
        ...
        or judge_manifest_sha256 != _sha256_bytes(_canonical_json_bytes(judge_identity))
    ):
    ...
    # The manifest identity checked above binds the complete index list, and
    # the canonical publisher already validated each index/evidence pair.
    # Avoid reserializing the million-row list merely to repeat indices_sha256.
    ...
    # The reducer still needs the selected candidate identities, but only this
    # compact selected JSONL is read.  Raw source corpora and model-generation
    # chunks are never reopened during finalization.
