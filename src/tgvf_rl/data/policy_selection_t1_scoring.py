    ...
    for exclusion in quality_config["exclusions"]:
        ...
        if reason == "source_ground_truth_truncated":
            ...
            if set(exclusion) != _T1_PATHOLOGICAL_EXCLUSION_FIELDS:
                raise ValueError("pathological-generation exclusion fields differ")
            if (
            ...
            for waiver in waivers:
                ...
                if (
                    ...
                    or not 0 <= attempt_index < T1_ATTEMPTS
                ):
                    raise ValueError(
                ...
                if expected_request_id in length_waivers:
                    raise ValueError("duplicate pathological-generation request waiver")
...
def _validate_pathological_length_waiver(
    ...
    *,
    ...
    if (
        ...
        or waiver.get("terminal_sampled_token_count") != evidence.sampled_token_count
...
def _apply_quality_exclusion(
    ...
    route = {
        ...
        T1_PATHOLOGICAL_GENERATION_LENGTH_REASON: ("source_generation_anomaly_invalid"),
    }[reason]
    return {
        **attempt,
        ...
        report.update(
            {
                ...
                "pathological_generation_exclusion_attempt_count": sum(
                    item["verification_route"] == "source_generation_anomaly_invalid"
                    for item in attempts
                ),
            }
        )
