...
c_evaluation_id=PRL25-C-FROZEN-RP67-TFREE-TEACHER25-COREDEV2511-S64-SEED43-REPLICATE-V1
d_evaluation_id=PRL25-D-ATOMIC-CROP-TGVF-RP67-TFREE-TEACHER25-COREDEV2511-S16-SEED43-REPLICATE-V1
c_plan="$repo_root/configs/evaluation/prl25_c_frozen_rp67_tfree_teacher25_s64_seed43_replicate_coredev2511_plan.json"
d_plan="$repo_root/configs/evaluation/prl25_d_atomic_crop_tgvf_tfree_teacher25_s16_seed43_replicate_coredev2511_plan.json"
c_evaluation_root="$c_training_root/evaluation/$c_evaluation_id"
d_evaluation_root="$d_training_root/evaluation/$d_evaluation_id"
control_root="$artifact_root/evaluation/PRL25-CD-SELECTED-SEED43-REPLICATES-V1/runtime/supervisor"
max_restarts=${PRL25_CD_SEED43_EVAL_MAX_RESTARTS:-10}
cooldown_seconds=${PRL25_CD_SEED43_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$c_evaluation_root/logs" "$d_evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL25 C/D seed43 replicate evaluator is active" >&2
  exit 1
fi
...
if [[ "$c_code" != 0 || "$d_code" != 0 ]]; then
  echo "PRL25 C/D seed43 evaluation failed: C=$c_code D=$d_code" >&2
  exit 1
fi
