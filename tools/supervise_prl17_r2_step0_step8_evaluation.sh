...
plan="$repo_root/configs/evaluation/prl17_r2_frozen_rp67_calibrated_t_step0_step8_coredev2511_plan.json"
training_root=/nvmesv/dredvpn009/projects/r-vlm/tgvf-e2e-rl/artifacts/policy/PRL-17-R2-qwen3-instruct-full-frozen-rp67-bs16-n16-t1-shaped-rp67t-novisual-8step-ws8
evaluation_id=PRL17-R2-FROZEN-RP67-CALIBRATED-T-COREDEV2511-STEP0-STEP8-SAME-PROTOCOL-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL17_R2_EVAL_MAX_RESTARTS:-4}
cooldown_seconds=${PRL17_R2_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL17-R2 evaluation supervisor is active" >&2
  exit 1
fi
