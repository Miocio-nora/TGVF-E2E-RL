...
plan="$repo_root/configs/evaluation/prl22_a_frozen_rp67_tfree_teacher25_step8_step16_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL22-A-FROZEN-RP67-TFREE-TEACHER25-COREDEV2511-STEP8-STEP16-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL22_A_PAIRED_EVAL_MAX_RESTARTS:-6}
cooldown_seconds=${PRL22_A_PAIRED_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL22-A paired evaluator is active" >&2
  exit 1
fi
