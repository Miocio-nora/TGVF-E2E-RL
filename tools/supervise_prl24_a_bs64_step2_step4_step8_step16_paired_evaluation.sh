...
plan="$repo_root/configs/evaluation/prl24_a_frozen_rp67_tfree_teacher25_bs64_step2_step4_step8_step16_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL24-A-FROZEN-RP67-TFREE-TEACHER25-BS64-COREDEV2511-STEP2-STEP4-STEP8-STEP16-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL24_A_EVAL_MAX_RESTARTS:-8}
cooldown_seconds=${PRL24_A_EVAL_RESTART_COOLDOWN_SECONDS:-60}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL24-A paired evaluator is active" >&2
  exit 1
fi
...
while true; do
  ...
  if rg -q "$fatal_pattern" "$attempt_log"; then
    echo "PRL24-A evaluation hit a deterministic contract failure" >&2
    exit "$code"
  fi
  if (( attempt >= max_restarts )); then
    echo "PRL24-A evaluation retry budget exhausted after $attempt attempts" >&2
    exit "$code"
  fi
  echo "recoverable PRL24-A evaluation failure; retrying in ${cooldown_seconds}s" >&2
