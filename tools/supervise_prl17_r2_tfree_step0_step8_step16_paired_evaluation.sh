...
plan="$repo_root/configs/evaluation/prl17_r2_frozen_rp67_tfree_step0_step8_step16_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL17-R2-FROZEN-RP67-TFREE-COREDEV2511-STEP0-STEP8-STEP16-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL17_R2_PAIRED_EVAL_MAX_RESTARTS:-4}
cooldown_seconds=${PRL17_R2_PAIRED_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another RP67 T-free three-arm paired evaluator is active" >&2
  exit 1
fi
