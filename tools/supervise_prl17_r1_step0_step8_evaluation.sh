...
plan="$repo_root/configs/evaluation/prl17_r1_frozen_rp67_step0_step8_coredev2511_plan.json"
...
evaluation_id=PRL17-R1-FROZEN-RP67-COREDEV2511-STEP0-STEP8-SAME-PROTOCOL-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL17_R1_EVAL_MAX_RESTARTS:-4}
cooldown_seconds=${PRL17_R1_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL17-R1 evaluation supervisor is active" >&2
  exit 1
fi
