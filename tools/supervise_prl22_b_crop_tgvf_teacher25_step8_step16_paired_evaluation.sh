...
plan="$repo_root/configs/evaluation/prl22_b_r0_frozen_rp67_tfree_crop_tgvf_teacher25_step8_step16_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL22-B-R0-FROZEN-RP67-TFREE-CROP-TGVF-TEACHER25-COREDEV2511-STEP8-STEP16-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL22_B_PAIRED_EVAL_MAX_RESTARTS:-6}
cooldown_seconds=${PRL22_B_PAIRED_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL22-B paired evaluator is active" >&2
  exit 1
fi
...
while true; do
  ...
  if rg -q "$deterministic_error_pattern" "$attempt_log"; then
    echo "deterministic PRL22-B evaluation contract failure; refusing retry" >&2
    exit "$code"
  fi
  if (( attempt > max_restarts )); then
    echo "PRL22-B evaluation retry budget exhausted after $attempt attempts" >&2
    exit "$code"
  fi
  echo "recoverable PRL22-B evaluation failure; retrying in ${cooldown_seconds}s" >&2
