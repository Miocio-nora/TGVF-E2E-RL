...
plan="$repo_root/configs/evaluation/prl18_r0_joint_rp67_tfree_step8_step16_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL18-R0-JOINT-RP67-TFREE-COREDEV2511-STEP8-STEP16-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL18_R0_PAIRED_EVAL_MAX_RESTARTS:-4}
cooldown_seconds=${PRL18_R0_PAIRED_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL18-R0 joint-RP67 two-arm paired evaluator is active" >&2
  exit 1
fi
