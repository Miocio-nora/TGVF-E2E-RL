...
plan="$repo_root/configs/evaluation/prl25_c_frozen_rp67_tfree_teacher25_s8_s16_s32_s48_s64_s80_paired_seed_coredev2511_plan.json"
...
evaluation_id=PRL25-C-FROZEN-RP67-TFREE-TEACHER25-COREDEV2511-S8-S16-S32-S48-S64-S80-PAIRED-SEED-V1
evaluation_root="$training_root/evaluation/$evaluation_id"
control_root="$evaluation_root/runtime/supervisor"
max_restarts=${PRL25_C_PAIRED_EVAL_MAX_RESTARTS:-10}
cooldown_seconds=${PRL25_C_PAIRED_EVAL_RESTART_COOLDOWN_SECONDS:-30}

mkdir -p "$control_root" "$evaluation_root/logs"
exec 9>"$control_root/supervisor.lock"
if ! flock -n 9; then
  echo "another PRL25-C six-point evaluator is active" >&2
  exit 1
fi
