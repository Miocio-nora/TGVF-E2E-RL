...
def _materialize_eval_fixture(
    root: Path,
    *,
    model: str = COREDEV_BASELINE_MODEL,
) -> dict[str, Path]:
    artifacts = {}
    for spec in COREDEV_2511.slices:
        dataset_root = root / spec.vlmeval_dataset
        run_dir = dataset_root / model / "T20260721-150000"
        run_dir.mkdir(parents=True)
        prediction = run_dir / f"{model}_{spec.vlmeval_dataset}.tsv"
        ...
        if judge_contract != "none_rule_based":
            ...
            artifact = run_dir / (
                f"{model}_{spec.vlmeval_dataset}_{COREDEV_LLM_JUDGE_MODEL}_result.tsv"
            )
...
def test_aggregate_preserves_instruct_policy_scoring_identity(tmp_path: Path) -> None:
    _materialize_eval_fixture(tmp_path, model=POLICY_SCORING_MODEL)
    ...
    assert result["model"] == POLICY_SCORING_MODEL
    assert all(POLICY_SCORING_MODEL in item["status_path"] for item in result["slices"])


def test_aggregate_does_not_disguise_instruct_results_as_thinking(
    tmp_path: Path,
) -> None:
    _materialize_eval_fixture(tmp_path, model=POLICY_SCORING_MODEL)

    with pytest.raises(RuntimeError, match="no eval status"):
        summarize_coredev_results(
            work_dir=tmp_path,
            repository_root=tmp_path,
            phase="eval",
