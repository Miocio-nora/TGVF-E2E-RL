...
def test_one_schedule_dispatches_without_changing_rows(
    ...
    samples = tuple(
        ...
        for index, source in enumerate(
            ("vstar", "arxivqa", "thinklite", "teacher")
        )
    )
    ...
    monkeypatch.setattr(
        module, "POLICY_PILOT_V1_CHAT_TEMPLATE_SHA256", template_sha256
    )
    binding = _binding(
        tmp_path,
        profile=profile,
    ...
    assert rows[0]["agent_name"] == visual_agent
    assert rows[1]["agent_name"] == visual_agent
    assert rows[3]["agent_name"] == visual_agent
...
def test_legacy_splits_preserve_schedule_identity_and_smoke_semantics(
    ...
    assert all(
        row["extra_info"]["mixture_role"] == "smoke" for row in smoke_rows
    )
