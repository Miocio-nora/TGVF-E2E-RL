...
def test_fused_selected_logprobs_match_eager_values_and_gradients() -> None:
    ...
    eager = _eager_selected_logprobs(eager_hidden, eager_head, token_ids, positions)
...
def test_microbatch_materializer_reuses_one_weight_and_sums_row_gradients(
    ...
    fused_rows = [row.detach().clone().requires_grad_(True) for row in eager_rows]
...
def _run_dtensor_materializer_regression(
    ...
    try:
        ...
        actual_hidden = [torch.randn(1, 6, 5, requires_grad=True) for _ in range(2)]
        expected_hidden = [
        ...
        hidden_grads_match = all(
            ...
            for actual, expected in zip(actual_hidden, expected_hidden, strict=True)
        )
        output_queue.put(
            (
                rank,
