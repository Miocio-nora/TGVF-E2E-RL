...
def _lifecycle(
    ...
    *,
    ...
    return PolicyCheckpointLifecycle(
        ...
        permanent_root=(root.parent / "permanent-checkpoints") if permanent_steps else None,
        world_size=2,
        run_identity=identity,
...
def test_restart_scan_prunes_before_save_and_keeps_latest_tracker(tmp_path: Path) -> None:
...
def test_foreign_or_partial_generation_is_never_a_retention_deletion_candidate(
    ...
    root = tmp_path / "checkpoints"
    identity = _identity()
    foreign = _materialize_generation(
        root, 1, identity=_identity("another-run")
    )
