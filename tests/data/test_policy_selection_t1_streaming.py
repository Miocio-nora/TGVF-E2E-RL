...
def test_legacy_index_presence_supports_flat_and_sharded_layouts(
    ...
    flat.unlink()
    sharded = tmp_path / "requests" / payload_sha256[:2] / f"{payload_sha256}.json"
...
def test_legacy_index_presence_rejects_non_file_claim(tmp_path: Path) -> None:
    ...
    claimed_index = (
        tmp_path / "requests" / payload_sha256[:2] / f"{payload_sha256}.json"
    )
...
def test_completed_index_inventory_supports_sharded_and_flat_layouts(
    ...
    flat_sha256 = "cd" + "2" * 62
    sharded = tmp_path / "requests" / "ab" / f"{sharded_sha256}.json"
