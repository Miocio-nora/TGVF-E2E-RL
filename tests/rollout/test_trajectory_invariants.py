...
def test_invalid_format_accepts_only_an_unexecuted_final_tool_marker() -> None:
    store, behavior_store, trajectory = _trajectory()
    ...
    final_error = ToolErrorRecord(
        attempt_index=1,
        assistant_turn_index=final_turn.turn_index,
    ...
    executed = replace(invalid, tool_errors=(final_error,))
    with pytest.raises(ReplayMismatchError, match="unexecuted final tool marker"):
        TrajectoryValidator(store, behavior_store).validate(executed)
